 ...
 * a friend circle. The 2nd student himself is in a friend circle. So return 2.
 *
 * Example 2:
 * Input: 
 * [[1,1,0],
 ...
 * 2nd students are direct friends, so the 0th and 2nd students are indirect 
...
namespace CSharpProblems
{
    public class Problem_547
    {
        public class Solution
        {
            ...
            private void DFS(int startIndex, int[][] chart, bool[] visited)
            {
                visited[startIndex] = true;

                for (int i = 0; i < chart[startIndex].GetLength(0); i++)
                {
                    if (startIndex != i)
                    {
                        if (chart[startIndex][i] == 1 && !visited[i])
                        {
                            DFS(i, chart, visited);
                        }
                    }
