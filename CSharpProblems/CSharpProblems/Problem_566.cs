 ...
 * Difficulty: Easy
 * 
 * In MATLAB, there is a useful function called 'reshape', which can reshape 
    ...
    public class Problem_566
    {
        public class Solution
        {
            public int[,] MatrixReshape(int[,] nums, int r, int c)
            {
                ...
                if (nums == null || nums.Length == 0 || r * c != row * col)
                {
                    return nums;
                }
                ...
                for (int i = 0; i < r; i++)
                {
                    for (int j = 0; j < c; j++)
                    {
                        res[i, j] = nums[originalX, originalY];
                        originalY++;
 ...
 * 						   once only. Here, m and n refers to the number of 
 * 						   rows and columsn in the given matrix.
