 ...
 * once and "wiki.org" 5 times. For the subdomains, we will visit 
        ...
        public class Solution
        {
            public IList<string> SubdomainVisits(string[] cpdomains)
            {
                ...
                for (int i = 0; i < cpdomains.Length; i++)
                {
                    ...
                    int count = Int32.Parse(cpdomain[0]);

                    string[] domainParts = cpdomain[1].Split('.');
                    string concatDomain = String.Empty;

                    for (int j = domainParts.Length - 1; j >= 0; j--)
                    {
                        concatDomain = String.IsNullOrEmpty(concatDomain) ? domainParts[j] : domainParts[j] + "." + concatDomain;
                        if (dict.ContainsKey(concatDomain))
                        {
                            dict[concatDomain] += count;
                        }
                        else
                        {
                            dict[concatDomain] = count;
...
/*
 * Complexity Analysis
 * 		Time Complexity: O(n), where N is the length of cpdomains. Assume length of cpdomains[i] is fixed (per notes).
